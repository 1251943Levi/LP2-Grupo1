 ...
 *
 * Formato das colunas (Sprint 6):
 *   sigla; nome; anoCurricular; siglaDocenteResponsavel; siglaCurso; ects; momentos
 *
 * A coluna "momentos" (índice 6) é retrocompatível:
 *   registos sem ela são lidos como 0 (não definido).
...
public class UcDAL {
    ...
    /**
     * Devolve um array "SIGLA - Nome" das UCs associadas a um curso específico.
     * Usado pelo AnoLetivoBLL para verificar quais UCs de um curso têm momentos definidos.
     *
     * Sprint 6 — tarefa 3.
     */
    public static String[] obterListaUcsPorCurso(String siglaCurso, String pastaBase) {
        String caminho = pastaBase + File.separator + NOME_FICHEIRO;
        List<String> linhas = DALUtil.lerFicheiro(caminho);
    ...
    /**
     * Devolve o número de momentos de avaliação definidos para a UC.
     * Retrocompatível: devolve 0 se a coluna não existir ou estiver vazia.
     *
     * Sprint 6 — tarefa 3 e 5.
     */
    public static int obterMomentos(String siglaUc, String pastaBase) {
        String caminho = pastaBase + File.separator + NOME_FICHEIRO;
        List<String> linhas = DALUtil.lerFicheiro(caminho);
     ...
     * Inclui a coluna momentos com valor 0 por omissão (não definido).
     */
    public static void adicionarUC(UnidadeCurricular uc, String siglaCurso, String pastaBase) {
        ...
        // coluna momentos = 0 (não definido até o gestor configurar)
    ...
    /**
     * Atualiza o número de momentos de avaliação de TODAS as linhas da UC.
     * A mesma UC pode ter várias linhas (uma por curso); todas são actualizadas.
     *
     * Sprint 6 — tarefa 5.
     */
    public static void atualizarMomentos(String siglaUc, int numMomentos, String pastaBase) {
        String caminho = pastaBase + File.separator + NOME_FICHEIRO;
        ...
        for (String linha : linhasAntigas) {
            if (linha.toLowerCase().startsWith("sigla")) {
                // Garantir cabeçalho com coluna momentos
                linhasAtualizadas.add(CABECALHO);
                continue;
            }
            String[] dados = linha.split(";", -1);
            if (dados.length >= 1 && dados[0].trim().equalsIgnoreCase(siglaUc)) {
                // Reconstruir com 7 colunas, preservando os valores existentes
     ...
     * Se for a última linha da UC, mantém a linha mas limpa o curso.
     */
    public static boolean removerAssociacaoUcCurso(String siglaUc, String siglaCurso, String pastaBase) {
        ...
        for (String linha : linhas) {
            ...
            if (dados.length >= 5
                ...
                encontrou = true;
                if (totalLinhasUC == 1) {
                    // última linha — manter UC mas limpar curso
                    dados[4] = "";
                    StringBuilder sb = new StringBuilder();
                ...
                // se houver mais linhas, apaga esta
                continue;
            }
            novasLinhas.add(linha);
        }
